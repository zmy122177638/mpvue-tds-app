<template>
  <section class="spec-box">
    ...
    <section class="spec-list-box">
      <div class="list-type-box" v-for="(item,i) in spec" :key="i">
        <header class="title">{{item.spec_name}}</header>
          <block v-for="(val,index) in item.spec_attr" :key="index">
            <div v-if="val === userSelected[i]" class="list-item active">
              {{val}}
            </div>
            <div v-else class="list-item" :key="index" @click="handleSelected(i,index,val)">
              {{val}}
            </div>
          </block>
      </div>
    ...
    <section class="num-box">
      ...
      <span class="num-right">
        <span v-if="selectedNum > 1" class="minus" @click="handleMinusNum">-</span>
        <span v-else class="minus disable">-</span>
        <span class="number">{{selectedNum}}</span>
        <span v-if="selectedNum <= limit_num" class="add" @click="handleAddNum">+</span>
        <span v-else class="add disable">+</span>
      </span>
    </section>
    <section class="btn-box">
      <button type="warn" @click="handleSubmitOrder">提交订单</button>
    </section>
  </section>
</template>
